...
# ╔═╡ f0bdd82c-0414-4b7d-a018-8398c11637c5
@info "" polytope=polytope
...
still_missing(md"Because we have the objective function as 

$$f(\theta; x) = \arg\max_{y\in\mathcal{Y}(x)} \theta^\top y$$

And the jacobian is calculated using:

```math
J=\left[ \begin{array}{ll} \frac{\partial f_1}{\partial \theta_1} & \frac{\partial f_1}{\partial \theta_2} \\\frac{\partial f_2}{\partial \theta_1} & \frac{\partial f_2}{\partial \theta_2}\end{array}\right]
```

Here, 

- In jacobian, $f_1$means the $x$ axis component
- In jacobian, $f_2$means the $y$ axis component
- In jacobian, $\theta_1$means the $cos$ component
- In jacobian, $\theta_2$means the $sin$ component

And considering that the $f(\theta;x)$ is an argmax function, without a pertubation, the value of $f(\theta;x)$ is always a **piecewise constant**. That results to a zero gradient in all parts in the jacobian considering $\theta$."
...
let
	θ = 0.5 .* [cos(angle_perturbed), sin(angle_perturbed)]
	j=Zygote.jacobian(θ -> perturbed_layer(θ; polytope), θ)[1]
	@info "" j=j θ=θ
end
...
这一函数式关于  的期望，而对于期望的直接求梯度是特别困难的。
因此在这一perturbed maximizer中，他采用了蒙特卡洛方法先对期望进行估算，进而可以对估计值   进行梯度的计算。

```math
\hat{f}_\varepsilon(\theta) \approx\frac{1}{n}\sum_{n=1}^n\underset{y\in \mathcal{Y}}{max}(\theta + \varepsilon Z)^\top y\\

\nabla \hat{f}_\varepsilon(\theta)
```

因此在这一情况下，对于不同的 、theta值，在jacobian中有连续变化的derivatives。
...
# ╔═╡ 06105e4d-dbd6-4981-8816-aafa03f83df0


# ╔═╡ 5d70fcf1-ef1a-4a3f-a505-aa2083d36b0c
...
# ╔═╡ b7d23acc-fd9e-4f8b-90b2-c3e8b4d5bda4
still_missing(md"
```math
\mathcal{L}_{\varepsilon}^{\text{FY}}(\theta, \bar{y}) = \mathbb{E}_{Z}\big[ \operatorname{max}_{y \in \mathcal{Y}(x)} \theta ^\top y \big]+\sup_y \{\theta^T\bar y-\mathbb{E}_{Z}\big[ \operatorname{max}_{y \in \mathcal{Y}(x)} \theta ^\top y]  \}-\theta^T\bar y

```
```math
=\operatorname{max}_{y \in \mathcal{Y}(x)} \theta ^\top y- \theta^T\bar y- \sup_y \{\theta^T\bar y-\operatorname{max}_{y \in \mathcal{Y}(x)} \theta ^\top y \} 
```
")

# ╔═╡ 0c8be1a0-8873-41fa-adc6-c423fd8b3d87


# ╔═╡ ff1dd919-3a71-4cab-9c20-187d98284511
...
still_missing(md"ChatGPT+知乎 防止和别的同学回答一样的 Dijkstra")
...
still_missing(md"ChatGPT+知乎 防止和别的同学回答一样的")
...
still_missing(md"
Gap of the given solution and the corresponding optimal solution is:
```math
g(\hat y,y)=c(\hat y)-c(y)

```
Having $r(\hat{y},y) = c(\hat{y}) / c(y)$, we can have:
```math
r(\hat{y},y)=1+\frac{g(\hat y,y)}{c(y)}≥1
```
Then we can see 

相似：
$r(\hat y,y)=1$ is equal to $g(\hat y,y)=0$, $\min r(\hat y,y)$ is equal to $\min g(\hat y,y)$
...
still_missing(md"您提到的两个扰动（perturbation）表达式似乎是在机器学习或统计学领域中用于优化或模型鲁棒性研究的公式。让我们先解释这两个表达式：

1. **Addictive Perturbation**: 
   \(\hat{f}_\varepsilon(\theta) = \mathbb{E}_{Z}\big[ \underset{y \in \mathcal{Y}}{\mathrm{argmax}} (\theta + \varepsilon Z)^\top y \big]\)
   
   这个表达式描述了一种添加性扰动，其中 \(\theta\) 是模型参数，\(\varepsilon\) 是扰动强度，\(Z\) 是随机变量。该扰动直接加到参数上，反映了模型参数在随机扰动下的响应。

2. **Element-wise Product Perturbation**: 
   \({y}_\varepsilon^\odot (\theta) := \mathbb{E}_Z \bigg[\operatorname{argmax}_{y \in \mathcal{C}} (\theta \odot e^{\epsilon Z - \varepsilon^2 \mathbf{1} / 2})^\top y \bigg]\)

   这个表达式描述了一种元素对元素乘积的扰动。这里使用了指数函数来调整扰动的影响，并通过元素对元素的乘积（element-wise product）将扰动应用于参数。

关于元素对元素乘积的扰动（Element-wise Product Perturbation）的优势，主要包括：

- **非线性调整**：通过 \(e^{\epsilon Z - \varepsilon^2 \mathbf{1} / 2}\) 应用非线性变换，可以提供比简单线性加法更复杂和灵活的扰动方式。
- **细粒度控制**：元素对元素乘积允许对每个参数单独调整扰动的强度，这在处理具有不同敏感度或重要性的参数时非常有用。
- **保留参数符号**：通过乘积方式，可以在扰动过程中保留原始参数的符号（正或负），这对于保持参数意义很重要。

在实际应用中，选择哪种扰动方式取决于具体的模型结构、目标任务和扰动的目的。例如，在一些对参数精细调整非常敏感的模型中，元素对元素乘积的扰动可能更加合适。
...
danger(md"Click the checkbox to activate the training cell $(@bind train_exp CheckBox()) 
...
danger(md"Click the checkbox to activate the training cell $(@bind train_exp_reg CheckBox()) 
